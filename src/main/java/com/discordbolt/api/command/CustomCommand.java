...
public class CustomCommand {
    ...
    private Logger logger = LoggerFactory.getLogger(CustomCommand.class);
    ...
    public String getUsage(IGuild guild) {
        return manager.getCommandPrefix(guild) + usage;
    }
    ...
    public void preexec(IMessage message, IUser user) {
        ...
        // Permission check
        if (!cc.getAuthor().getClient().getApplicationOwner().equals(cc.getAuthor())) {
        ...
        try {
            ...
            if (ite.getCause() instanceof CommandException) {
                ...
                logger.error("Uncaught exception during execution of \"" + manager.getCommandPrefix(cc.getGuild()) + String.join(" ", getCommands()) + "\" command.");
